...
Tree is an implementation of the [Nested Set](http://en.wikipedia.org/wiki/Nested_set_model) pattern for [Laravel 5's](http://laravel.com/) Eloquent ORM.
...
As with most Laravel 5 packages you'll then need to register the Tree
...
configured to work with the Nested Set behaviour provided by Tree. You SHOULD
...
In order to work with Tree, you must ensure that your model class extends
`Tree\Node`.
...
You must ensure that the database table that supports your Tree models has the
following columns:
...
to use Tree with your model. Below are some examples.
...
Tree provides several methods for moving nodes around:
...
You can ask some questions to your Tree nodes:
...
Tree provides two self-referential Eloquent relations for your nodes: `parent`
and `children`.
...
Tree provides some very basic query scopes for accessing the root and leaf nodes:
...
There are several methods which Tree offers to access the ancestry/descendancy
...
Second, as **methods** which return actual `Tree\Node` instances (inside a `Collection`
...
In some situations where the hierarchy depth is huge it might be desirable to limit the number of levels of children returned (depth). You can do this in Tree by using the `limitDepth` query scope.
...
By default in Tree all results are returned sorted by the `lft` index column
...
Tree extends the default `Eloquent\Collection` class and provides the
...
Tree models fire the following events: `moving` and `moved` every time a node
...
Tree provides a simple method to provide Nested Set "scoping" which restricts
...
Tree supports for complete tree-structure rebuilding (or reindexing) via the
...
and Tree will try its best to recompute the rest.
...
Tree comes with **limited support** for soft-delete operations. What I mean
...
Because Nested Set structures usually involve a number of method calls to build a hierarchy structure (which result in several database queries), Tree provides two convenient methods which will map the supplied array of node attributes and create a hierarchy tree from them:
...
Tree provides some query scopes which may be used to extract (remove) selected nodes
...
asked questions about Tree in the [wiki](https://github.com/etrepat/baum/wiki).
...
Tree is licensed under the terms of the [MIT License](http://opensource.org/licenses/MIT)
